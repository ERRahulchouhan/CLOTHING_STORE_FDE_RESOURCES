...
- **Chat with the assistant** in the corner of the screen. Ask it in normal
  English ("anything for kids under 1000?") and it returns real products from
  the catalog.
...
| **Understand natural language** | Instead of writing a rules engine, the assistant uses a language model that already understands English and can *call a function* (`search_products`) with the right filters. |
| **Stay safe** | Two dedicated safety models screen every message and every reply, so the assistant can't be tricked into leaking its instructions or saying something harmful. |
| **See what's happening** | Every web request and every AI step is traced to Pydantic Logfire, so problems are visible. |
...
graph TD
    ...
    subgraph "Backend (Python)"
        ...
        AGENT["Pydantic AI agent<br/>the shopping assistant"]
        GUARD["Guardrail models<br/>safety checks"]
    end
    DB[("MongoDB Atlas<br/>products, carts,<br/>orders, users")]
    GROQ["Groq<br/>runs the language models"]
    LOG["Pydantic Logfire<br/>tracing / monitoring"]
...
| **AI agent framework** | Pydantic AI | Lets a language model reliably call a Python function and return structured data instead of loose text. |
| **Language‑model provider** | Groq | Runs the actual models — the shopping agent's model and the two guardrail models. |
| **LLM gateway (optional)** | Portkey | If configured, all model calls are routed through it for extra logging and reliability; otherwise the app calls Groq directly. |
...
- **Groq** — every language‑model call. The app never runs a model itself.
- **Portkey** — *optional.* A proxy in front of Groq for logging/retries.
- **Pydantic Logfire** — *optional.* Receives traces. Without a token the app
  still runs; it just doesn't send anything.
- **Google Identity Services** — issues the sign‑in token the "Sign in with
  Google" button produces.
- **Google Cloud** — hosts the running app. See [Deployment](07-deployment.md).
